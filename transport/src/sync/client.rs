...
use idl_rs::store::sync::ids::safe_join;
...
/// peer down) or malformed JSON is a typed [`TransportError`] — this is one
/// of the two failures that aborts the whole run (see [`sync_with_peer`]'s
/// doc comment).
async fn fetch_manifest(client: &reqwest::Client, base_url: &str, token: &str) -> Result<Manifest, TransportError> {
    ...
    response.json::<Manifest>().await.map_err(|e| sync_error(format!("GET /manifest returned malformed JSON: {e}")))
}
...
/// starts the file fresh. A `206` whose start does not match what was
/// asked — the peer resumed somewhere else — is refused rather than risking
/// corrupting the partial file by appending misaligned bytes.
async fn download_item(client: &reqwest::Client, base_url: &str, token: &str, data_root: &Path, item: &SyncItem) -> Result<Vec<u8>, TransportError> {
    ...
    let append = if status == reqwest::StatusCode::PARTIAL_CONTENT {
        ...
        if start_byte != existing_len {
            return Err(sync_error(format!("GET {url}: peer resumed at byte {start_byte}, but {existing_len} was requested")));
        }
        true
    } else {
        false
...
/// and removes the now-consumed `.part` file only after `install` succeeds.
/// A `.part` file is left in place on any failure (network or install) so a
/// later run can resume or retry it.
async fn pull_and_install(
    ...
    let ctx = build_install_context(item, remote_manifest)?;
    let outcome =
        install(data_root, item, &bytes, &peer.name, now_ms, &ctx).map_err(|e| sync_error(format!("install {:?} {}: {e}", item.class, item.key)))?;
    let tmp_path = tmp_part_path(data_root, item);
